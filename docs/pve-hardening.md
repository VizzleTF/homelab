...
До сих пор PVE-хосты не были автоматизированы: root-only SSH, без firewall на датацентре,
без fail2ban, без централизованного мониторинга состояния самих гипервизоров. Если нода pve
упадёт или будет скомпрометирована — узнаем случайно. Единственная компенсация сейчас — это
замкнутая домашняя сеть и ручная дисциплина.

Блок 2 этого плана закрывает host security (admin-пользователь, SSH-ключи, fail2ban, sysctl,
datacenter firewall). Блок 3 добавляет etcd-snapshot-бэкапы Talos и экспортёр метрик PVE в
Victoria Metrics.
...
| Роль | Что делает |
|------|-----------|
| `pve_admin_user` | Linux-пользователь `admin` + sudo + pveum user `admin@pam` с ролью Administrator |
| `pve_ssh_harden` | `sshd_config.d/99-homelab.conf`: запрет password auth, client alive, AllowUsers |
...
Порядок в `playbooks/pve_hosts.yaml`: **admin_user → ssh_harden → sysctl → fail2ban →
firewall**. Firewall последним, чтобы в случае бага хотя бы остальные роли применились.
...
- `terraform_proxmox/configs/pve_hosts.yaml` — хосты, admin-группа, trusted_cidrs.
  Из него модуль `modules/ansible_inventory` генерирует:
  - `ansible/inventory/inventory.yaml`
  - `ansible/inventory/group_vars/pve.yaml` (admin_ssh_pubkey = `TF_VAR_pc_public_key`)
...
# Наблюдение 5-10 минут: pve-firewall status, ssh admin@pve6, web UI на 8006
...
```bash
# admin-пользователь работает, root password-auth — нет
ssh admin@pve1 -i ~/.ssh/id_ed25519   # ок
ssh root@pve1                          # Permission denied (publickey)

pveum user list                        # admin@pam присутствует с ролью Administrator
...
- **admin user**: `pveum user delete admin@pam && userdel -r admin`
